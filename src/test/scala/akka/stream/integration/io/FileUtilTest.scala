...
class FileUtilTest extends TestSpec {
  "check file exists" should "find an existing file" in {
    Source.single(FileUtilsCommand.Exists("src/test/resources/application.conf"))
  ...
  "check file exists tagged" should "find an existing file" in {
    ...
    Source.single((tag, FileUtilsCommand.Exists("src/test/resources/application.conf")))
